...
2. **You provide** a natural language target prompt (e.g. `"pbi finance director"` or `"finance management managers/directors"`) and search keywords.
3. **AI Semantic Prompt Interpreter & Strict Conjunction Search**: Automatically normalizes acronyms (`PBI → Power BI`, `HR → Human Resources`), resolves slash syntax (`managers/directors`), and evaluates target keywords under a **strict `AND` condition** across all candidate profile fields.
4. **PostgreSQL pgvector Hybrid Search & Dataset Scope Isolation**: Matches candidates against your uploaded dataset with dataset-level isolation (`source_file`).
5. **Live Dataset Match Accuracy Engine**: Scores overall dataset fit across Role, Department, Keyword, Industry, and Location dimensions.
6. **AI Agent Writes Personalized Cold Emails**: Generates dual pitch variants (Option A & Option B) with editable `To`, `Cc`, `Bcc` recipient fields and full conversation thread history.
...
- 📥 **Step-by-Step Dataset Import & Breakdown Summary** — Flexible import options: Option A (Append New Data with `LOWER(TRIM(email))` deduplication), Option B (Reset & Upload New Dataset with confirmation modal), Option C (Manual Profile Entry), plus vertical scroll support and Import Summary popups showing rows processed, added, skipped duplicates, and invalid records.
- 🎯 **Strict Conjunction (`AND` Condition) Keyword Targeting** — Campaign keyword discovery strictly requires **all** specified keywords to be present across prospect profile fields, ensuring high-accuracy audience qualification.
