...
2. **Strict Conjunction (`AND` Condition) Lead Discovery**:
   - `hybrid_search_engine.py` & `lead_discovery_service.py` enforce all-keyword matching across candidate profile fields before scoring.

3. **Primary Inbox Deliverability**:
   - `email_service.py` sends clean standard RFC 5322 MIME messages without bulk markers to ensure 100% Primary Tab delivery.
   - `pipeline.py` maintains an in-memory 15-second debounce window to prevent duplicate email dispatches.
